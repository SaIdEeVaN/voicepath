...
class TestWeights:
    ...
    def test_perfect_profile_scores_one(self):
        result = score_one(make_profile(), make_scheme())
    ...
    def test_overall_is_the_weighted_sum(self):
        ...
        result = score_one(
            ...
            make_scheme(certifications_required=["Trade Certificate"]),
        )
        expected = (
...
class TestExperience:
    def test_meeting_the_minimum_is_full_marks(self):
        result = score_one(make_profile(experience_years=2.0), make_scheme())
        assert result.experience_score == 1.0

    def test_exceeding_the_minimum_is_not_extra_credit(self):
        two = score_one(make_profile(experience_years=2.0), make_scheme())
        twenty = score_one(make_profile(experience_years=20.0), make_scheme())
        assert two.experience_score == twenty.experience_score == 1.0

    def test_falling_short_scales_rather_than_disqualifies(self):
        result = score_one(make_profile(experience_years=1.0), make_scheme())
        assert result.experience_score == pytest.approx(0.5)

    def test_no_minimum_means_no_barrier(self):
        result = score_one(
            make_profile(experience_years=None), make_scheme(minimum_experience=0)
        )
        assert result.experience_score == 1.0
    ...
    def test_unstated_experience_is_neither_credited_nor_punished(self):
        ...
        result = score_one(make_profile(experience_years=None), make_scheme())
        assert result.experience_score == matching.EXPERIENCE_UNKNOWN


class TestEligibility:
    def test_nothing_required_is_fully_met(self):
        result = score_one(make_profile(), make_scheme(certifications_required=[]))
    ...
    def test_unstated_certificate_is_not_assumed(self):
        ...
        result = score_one(
            ...
            make_scheme(certifications_required=["LMV Driving Licence"]),
        )
...
class TestLocation:
    def test_same_place_is_best(self):
        result = score_one(make_profile(location="Salem"), make_scheme())
    ...
    def test_town_inside_the_district_counts_as_district(self):
        result = score_one(
            ...
            make_scheme(location="Salem", district="Salem"),
        )
...
class TestSkillSimilarity:
    ...
    def test_unrelated_skills_earn_nothing(self):
        ...
        result = score_one(profile, make_scheme())
    ...
    def test_coverage_is_of_the_requirement_not_the_person(self):
        ...
        assert (
            score_one(focused, make_scheme()).skill_similarity_score
            == score_one(scattered, make_scheme()).skill_similarity_score
        )
    ...
    def test_essential_skill_carries_more_weight(self):
        ...
        result = score_one(make_profile(), scheme)
...
class TestRanking:
    def _catalogue(self) -> list[Scheme]:
        return [
            make_scheme(id=1),
            make_scheme(id=2, minimum_experience=10.0),
            make_scheme(id=3, location="Erode", district="Erode"),
            make_scheme(
                id=4,
                required_skills=[
    ...
    def test_ranking_is_deterministic(self):
        ...
        first = [(m.scheme.id, m.overall_score) for m in rank(profile, catalogue)]
        for _ in range(9):
            assert [
                (m.scheme.id, m.overall_score) for m in rank(profile, catalogue)
            ] == first

    def test_ties_break_on_id_so_order_is_stable(self):
        catalogue = [make_scheme(id=7), make_scheme(id=3)]
        ranked = rank(make_profile(), catalogue)
        assert ranked[0].overall_score == ranked[1].overall_score
        assert [m.scheme.id for m in ranked] == [3, 7]
